        ...
        <section class="about">
            <div class="relative lg:p-6 py-16 lg:py-20 text-center z-50">
                ...
                <div class="relative flex justify-end mt-6">
                    <form method="GET" action="{{ route('workshop-enligne') }}" class="w-full max-w-lg">
                        <div class="bg-white shadow-lg rounded-lg p-6 flex space-x-4">
                            <!-- Sélecteur de catégorie avec recherche -->
                            <select name="category_id" id="category_id" onchange="this.form.submit()"
                                class="select2 w-full px-4 py-2 rounded-lg border border-gray-300 bg-gray-50 text-gray-700 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                            ...
                            <div
                                ...
                                @if ($EnLigne->image)
                                    <div class="flex justify-center items-center">
                                        <img src="{{ asset('storage/' . $EnLigne->image) }}" alt="{{ $EnLigne->name }}"
                                            class="h-72 lg:h-80 object-cover rounded-full scale-75 hover:scale-100 transition-transform duration-300">
                                    </div>
                                @else
                                    <div class="bg-gray-200 w-full h-72 flex items-center justify-center rounded-full">
